...
6. 向用户展示拟定的提交信息，通过交互按钮一步完成确认或修改（提供”确认提交”/”取消”选项，同时说明”如需修改，请直接在下方输入新的提交信息后发送”）：
   - 若用户选择”取消”，停止流程，不执行任何提交操作
   - 若用户发送了新的提交信息文本，直接使用该文本作为最终提交信息，继续执行后续步骤
   - 若用户选择”确认提交”，继续执行后续步骤
7. 使用以下命令创建提交：
   ```
   git commit -m “...”
   ...
   - 当需要分组提交时，按分组重复执行”暂存 -> 生成提交信息 -> 确认 -> 提交”，直到该组完成
8. 每次提交后重新运行 `git status` 检查剩余变更：
   - 若仍有未提交变更，继续下一组提交
   - 目标是确保本次要处理的所有变更最终都已提交（`git status` 不再显示相关未提交改动）
9. 所有提交完成后，先列出并告知用户本次新产生的提交信息（至少包含每条提交的哈希短 ID 与主题行，可使用 `git log --oneline` 按本次提交数量展示）。
10. 在告知提交信息后，必须通过交互按钮询问用户是否执行 `push`（提供”是/否”选项）：
