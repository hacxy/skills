...
| `detect-stage.sh` | 前置：检测项目当前阶段（接受 PROJECT_DIR 参数）|
...
```bash
# harness 自动验证：PRD 文件存在且有内容（验证通过自动 done，失败自动 fail）
...
```bash
# harness 自动验证（验证通过自动 done，失败自动 fail）
...
```
# UI Designer：每次最多 2 个页面，避免 agent 超时（500 server error）
# 先列出 design/ 下缺失的页面，按 2 个一组并行 spawn
# 例：5 个页面 → 第一批 spawn 2 个 agent（各 2 页）→ 第二批 spawn 1 个 agent（1 页）
...
**前提确认（总监负责，spawn agent 之前先执行）：**

```bash
gh auth status                          # gh CLI 已登录
cat ~/.config/ship/server.conf          # SSH_HOST、BASE_DOMAIN 等已填写

# 环境变量三处对齐检查（.env.example / deploy.yml / 代码）
bash "$SKILL_DIR/scripts/env-sync-check.sh" <project-dir>

# nginx 代理覆盖检查（/auth/ 等非 /api/ 路由是否有 location 块）
...
**harness 最终验证（verify-browser 对生产 URL）：**

```bash
source ~/.config/ship/server.conf
bun run ~/.claude/skills/ship/scripts/verify-browser.ts \
...
`validate-stage.sh` 失败会自动把阶段标记为 `failed`。重新调度 agent 修复前，**必须**先把状态重置为 `in_progress`：
