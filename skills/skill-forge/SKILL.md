...
description_zh: "创建新技能、修改和改进现有技能，并衡量技能性能。当用户想要从头创建技能、编辑或优化现有技能、运行评估测试技能、通过方差分析基准测试技能性能，或优化技能描述以提高触发准确性时使用。"
---

# Skill Forge

A skill for creating new skills and iteratively improving them. Works with any AI agent (Claude Code, OpenCode, Cursor, etc.).
...
The skill forge is liable to be used by people across a wide range of familiarity with coding jargon. If you haven't heard (and how could you, it's only very recently that it started), there's a trend now where the power of AI is inspiring plumbers to open up their terminals, parents and grandparents to google "how to install npm". On the other hand, the bulk of users are probably fairly computer-literate.
...
Understanding the triggering mechanism helps design better eval queries. Skills appear in the agent's `available_skills` list with their name + description, and the agent decides whether to consult a skill based on that description. The important thing to know is that agents only consult skills for tasks they can't easily handle on their own — simple, one-step queries like "read this PDF" may not trigger a skill even if the description matches perfectly, because agents can handle them directly with basic tools. Complex, multi-step, or specialized queries reliably trigger skills when the description matches.
...
This skill works with any AI agent. Here's how to adapt for different environments:

### Headless/Remote Environments

If you're in an environment without a browser or display:
- Use `--static <output_path>` to write a standalone HTML file instead of starting a server
- Feedback will be downloaded as a `feedback.json` file when the user clicks "Submit All Reviews"
- After download, copy `feedback.json` into the workspace directory for the next iteration to pick up

### No Subagents

If your environment doesn't support subagents:
- Run test cases one at a time instead of in parallel
- Skip baseline runs — just use the skill to complete the task as requested
- Skip quantitative benchmarking — focus on qualitative feedback from the user
- Present results directly in the conversation instead of using the browser viewer
...
Please add steps to your TodoList, if you have such a thing, to make sure you don't forget. If you're in a headless environment, please specifically put "Create evals JSON and run `eval-viewer/generate_review.py` so human can review test cases" in your TodoList to make sure it happens.
